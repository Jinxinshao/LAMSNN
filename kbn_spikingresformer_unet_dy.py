...
class DSSA(nn.Module):
    def __init__(self, dim, num_heads, spatial_size, patch_size, activation=LIF):
        super().__init__()
...
class UpsampleLayer(nn.Module):
    def __init__(self, in_channels, out_channels, scale_factor=2, activation=LIF) -> None:
        super().__init__()
        # 验证输入参数
        assert callable(activation), f"activation必须是可调用的类，而不是{type(activation)}"
...
class FeatureFusionBlock(nn.Module):
    """用于融合跳跃连接特征和上采样特征的块"""
    def __init__(self, skip_channels, up_channels, out_channels, activation=LIF):
        super().__init__()
        # 验证输入参数
        assert callable(activation), f"activation必须是可调用的类，而不是{type(activation)}"
...
class DyTKBNSNNUNet(nn.Module):
    """
    改进的带有动态Tanh的KBN-SNN-UNet模型
    
    此模型使用输入图像的变换而不是直接生成输出图像
    输出层生成6个通道，分为K和B，应用变换: x = K * x - B + x
    使用DyT (Dynamic Tanh)模块代替标准Tanh激活
    """
    def __init__(
        self,
        ...
        T=4,
        in_channels=3,
        out_channels=3,  # 这里保持为3，内部处理输出层为6通道
        prologue=None,
        group_size=64,
        activation=LIF,
        init_method='improved',
        init_alpha=1.0,  # 新增：DyT的初始alpha值
        **kwargs,
    ):
        super().__init__()
        # 验证关键输入参数
        ...
        self.init_alpha = init_alpha  # 存储初始alpha值
        ...
        # 修改：使用DynamicTanh代替标准Tanh
        # 对K和B各自使用单独的DynamicTanh实例
    ...
    def kbn_transform(self, x, feat):
        """应用KBN变换：x = K * x - B + x，使用DynamicTanh代替标准Tanh"""
        # 将特征分为K和B部分
        K, B = torch.split(feat, [self.out_channels, self.out_channels], dim=1)
        
        # 应用动态Tanh将K和B限制在[-1,1]范围内
        K = self.dyt_K(K)
        B = self.dyt_B(B)
...
# 修改现有的模型注册函数来使用新的DyTKBNSNNUNet
@register_model
def dyt_kbn_spikingresformer_unet_small(**kwargs):
    """基于SpikingResformer-S的DyT-KBN-UNet"""
    return DyTKBNSNNUNet(
        encoder_layers=[
            ['DSSA', 'GWFFN'] * 1,  # 阶段1
            ['DSSA', 'GWFFN'] * 2,  # 阶段2
            ['DSSA', 'GWFFN'] * 3,  # 阶段3
...
@register_model
def dyt_kbn_spikingresformer_unet_medium(**kwargs):
    """基于SpikingResformer-M的DyT-KBN-UNet"""
    return DyTKBNSNNUNet(
        encoder_layers=[
            ['DSSA', 'GWFFN'] * 1,  # 阶段1
            ['DSSA', 'GWFFN'] * 2,  # 阶段2
...
@register_model
def dyt_kbn_spikingresformer_unet_large(**kwargs):
    """基于SpikingResformer-L的DyT-KBN-UNet"""
    return DyTKBNSNNUNet(
        encoder_layers=[
            ['DSSA', 'GWFFN'] * 1,  # 阶段1
            ['DSSA', 'GWFFN'] * 2,  # 阶段2
